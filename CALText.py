...
def main(args):
  global data_folder_path  # Declare as global if needed

  if args.dataset == 'PUCIT_OHUP':
    data_folder_path=data_folder + 'PUCIT_OHUP/'
    ...
    return
  #### Loding Data ####
  train,train_uid_list = data.dataIterator(data_folder_path+'train_pages.pkl',data_folder_path+'train_labels.pkl', batch_size=BATCHSIZE, batch_Imagesize=BATCH_IMAGESIZE,maxlen=MAX_LEN, maxImagesize=MAXIMAGESIZE)
  valid, valid_uid_list = data.dataIterator(data_folder_path+'valid_pages.pkl',data_folder_path+'valid_labels.pkl',batch_size=BATCHSIZE, batch_Imagesize=BATCH_IMAGESIZE,maxlen=MAX_LEN, maxImagesize=MAXIMAGESIZE)
  test, test_uid_list = data.dataIterator(data_folder_path+'test_pages.pkl',data_folder_path+'test_labels.pkl',batch_size=1, batch_Imagesize=BATCH_IMAGESIZE,maxlen=MAX_LEN, maxImagesize=MAXIMAGESIZE)
  worddicts, vocabulary_count, space_ind=utility.load_dict_picklefile(data_folder_path+'vocabulary_unicode.pkl')
  ...
  with tf.Graph().as_default():
    	...
    	with tf.Session(config=config) as sess:
    	   ...
    	   ###Testing mode ###			
    		if(args.mode=='test'):
    			saver.restore(sess, model_folder + 'cp-0045-25.278371.ckpt') 
...
if __name__ == "__main__":
	...
	parser.add_argument("--dataset", default='PUCIT_OHUP')
